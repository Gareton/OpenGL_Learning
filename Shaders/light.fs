...
vec3 calcDirLight(DirLight l, vec3 normal, vec3 viewDir)
{
	...
	vec3 final_diffuse = texture(material.texture_diffuse1, TexCoords).rgb;
	vec3 final_specular = texture(material.texture_specular1, TexCoords).rgb;

	vec3 ambient = final_diffuse * l.ambient;	
	vec3 diffuse = diff * final_diffuse * l.diffuse;
	vec3 specular = final_specular * spec * l.specular;	
